  ...
  return (
    ...
    >
      <div className="flex flex-col h-full py-md">
        ...
        <div className="px-gutter mb-lg flex items-center justify-between gap-xs">
          {!isMinimized && (
        ...
        <ul className="flex flex-col flex-1 px-sm gap-xs">
          {navItems.map((item) => (
            <li key={item.label}>
              <a
                href={item.href}
                className={`flex items-center gap-sm px-sm py-sm rounded-lg transition-colors duration-200 active:scale-[0.98] justify-center ${
                  isMinimized ? 'px-1' : ''
                } ${
                  item.active
        ...
        {/* CTA */}
        <div className={isMinimized ? 'px-sm' : 'px-gutter'}>
          <button
